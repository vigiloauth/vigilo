...
type EmailService interface {
	...
	// GenerateEmailRequest populates an EmailRequest with necessary data based on the request type.
	//
	// Parameters:
	//
	...
	GenerateEmailRequest(request EmailRequest) *EmailRequest

	// SetTemplate sets the email template to be used for sending emails.
	//
	// Parameters:
	//
	//   tmplContent  string: The path to the email template.
	//
	// Returns:
	//
	//   error: An error if setting the template fails.
	SetTemplate(tmplContent string) error
